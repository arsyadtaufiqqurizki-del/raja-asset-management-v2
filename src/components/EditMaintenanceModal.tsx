...
export default function EditMaintenanceModal({ isOpen, onClose, record, onSuccess }: EditMaintenanceModalProps) {
  ...
  useEffect(() => {
    if (record) {
      ...
      setActualCost(record.actualCost && record.actualCost !== '--' ? record.actualCost.replace(/\D/g, '') : '');
    }
  }, [record]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/maintenance/${record.maintenanceId || record.id}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          status,
          actualCost: status === 'Completed' && actualCost ? `$${parseInt(actualCost.replace(/\D/g, ''), 10).toLocaleString('en-US')}` : record.actualCost || '--'
        })
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-scrim/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl w-full max-w-sm shadow-xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-outline-variant bg-surface-bright">
          <h2 className="text-xl font-bold text-on-surface">Update Status</h2>
