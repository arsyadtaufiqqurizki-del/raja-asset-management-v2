...
export default function AddAssetModal() {
  ...
  const [newAsset, setNewAsset] = useState({
    id: '', // Used as the primary asset key mapping to Asset Number
    assetNumber: '',
    assetDescription: '',
    assetBook: '',
    assetUnits: '1',
    assetCost: '',
    datePlacedInService: new Date().toISOString().split('T')[0],
    categorySegment1: '',
    categorySegment2: '',
    depreciationMethod: '',
    lifeInMonths: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalAsset = {
      ...newAsset,
      id: newAsset.assetNumber,
      name: newAsset.assetDescription,
      val: newAsset.assetCost.startsWith('$') ? newAsset.assetCost : `$${newAsset.assetCost}`,
      date: newAsset.datePlacedInService,
    };
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to save asset. Please check if the Asset Number already exists.');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-4xl max-h-[90vh] flex flex-col rounded-xl bg-surface-container-lowest shadow-xl border border-outline-variant">
        ...
        <div className="overflow-y-auto p-6 flex-1">
          <form id="add-asset-form" onSubmit={handleSubmit} className="flex flex-col gap-8">
            {errorMsg && (
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input required placeholder="e.g. RRPL/23/1.30.01/004" value={newAsset.assetNumber} onChange={handleInputChange('assetNumber')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold text-on-surface">Asset Description <span className="text-error">*</span></label>
                  <input required placeholder="e.g. Toyota Fortuner 2023" value={newAsset.assetDescription} onChange={handleInputChange('assetDescription')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold text-on-surface">Asset Book</label>
                  <input placeholder="e.g. RRPL" value={newAsset.assetBook} onChange={handleInputChange('assetBook')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold text-on-surface">Asset Units</label>
                  <input type="number" required placeholder="e.g. 1" value={newAsset.assetUnits} onChange={handleInputChange('assetUnits')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input required placeholder="e.g. 450000000" value={newAsset.assetCost} onChange={handleInputChange('assetCost')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold text-on-surface">Date Placed in Service <span className="text-error">*</span></label>
                  <input type="date" required value={newAsset.datePlacedInService} onChange={handleInputChange('datePlacedInService')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input placeholder="e.g. Kendaraan" value={newAsset.categorySegment1} onChange={handleInputChange('categorySegment1')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-semibold text-on-surface">Category Segment 2 (Location)</label>
                  <input placeholder="e.g. Thamres, Jakarta" value={newAsset.categorySegment2} onChange={handleInputChange('categorySegment2')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input placeholder="e.g. Straight Line" value={newAsset.depreciationMethod} onChange={handleInputChange('depreciationMethod')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none" />
                </div>
                <div className="flex flex-col gap-1.5">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <select value={newAsset.listedStatus} onChange={handleInputChange('listedStatus')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select value={newAsset.status} onChange={handleInputChange('status')} className="w-full px-3 py-2 bg-surface-container-low border border-outline-variant rounded-lg text-sm text-on-surface focus:border-primary focus:ring-1 focus:ring-primary outline-none">
